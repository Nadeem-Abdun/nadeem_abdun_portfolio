...
const AboutMeCard: React.FC<Props> = props => {
  ...
  const addProfileApiCall = async () => {
    try {
      ...
      if (selectedAboutMe.fullName)
        formData.append('fullName', selectedAboutMe.fullName);
      if (
        selectedAboutMe.professionalRoles &&
        selectedAboutMe.professionalRoles.length > 0
      )
        formData.append(
          'professionalRoles',
          JSON.stringify(selectedAboutMe.professionalRoles)
        );
      if (selectedAboutMe.introducingLine)
        formData.append('introducingLine', selectedAboutMe.introducingLine);
      if (profilePictureFile)
        formData.append('profilePicture', profilePictureFile);
      if (selectedAboutMe.primaryDescription)
        formData.append(
          'primaryDescription',
          selectedAboutMe.primaryDescription
        );
      if (selectedAboutMe.secondaryDescription)
        formData.append(
          'secondaryDescription',
          selectedAboutMe.secondaryDescription
        );
      if (selectedAboutMe.githubUrl)
        formData.append('githubUrl', selectedAboutMe.githubUrl);
      if (selectedAboutMe.linkedInUrl)
        formData.append('linkedInUrl', selectedAboutMe.linkedInUrl);
      if (selectedAboutMe.discordUrl)
        formData.append('discordUrl', selectedAboutMe.discordUrl);
      if (selectedAboutMe.twitterUrl)
        formData.append('twitterUrl', selectedAboutMe.twitterUrl);
      if (selectedAboutMe.mailToId)
        formData.append('mailToId', selectedAboutMe.mailToId);
  ...
  return (
    ...
    >
      <Grid container justifyContent="center" alignItems="center" rowGap={2}>
        ...
        >
          ...
          >
            {profile?.length === 0 ? (
              <Grid>
                <IconButton onClick={() => handleAboutMeDialogOpen('add')}>
                  <AddIcon className="text-cyan-400" />
                </IconButton>
              </Grid>
            ) : (
              <Grid>
                <IconButton onClick={() => handleAboutMeDialogOpen('edit')}>
                  <EditIcon className="text-cyan-400" />
                </IconButton>
              </Grid>
            )}
          </Grid>
        ...
        <Grid container size={{ xs: 12 }} flexDirection="column">
          ...
          <Grid>
            {professionalRoles &&
              professionalRoles.map((item, index) => {
                return (
                  <li
                    key={index}
        ...
        <Grid container size={{ xs: 12 }} flexDirection="column">
          ...
          <Grid>
            <Typography variant="body2" fontWeight={400} fontFamily="inter">
              {ellipsisString(githubUrl, 40)}
            </Typography>
          </Grid>
          <Grid>
            <Typography variant="body2" fontWeight={400} fontFamily="inter">
              {ellipsisString(linkedInUrl, 40)}
            </Typography>
          </Grid>
          <Grid>
            <Typography variant="body2" fontWeight={400} fontFamily="inter">
              {ellipsisString(discordUrl, 40)}
            </Typography>
          </Grid>
          <Grid>
            <Typography variant="body2" fontWeight={400} fontFamily="inter">
              {ellipsisString(twitterUrl, 40)}
            </Typography>
          </Grid>
          <Grid>
            <Typography variant="body2" fontWeight={400} fontFamily="inter">
              {ellipsisString(mailToId, 40)}
            </Typography>
          </Grid>
        </Grid>
      </Grid>
      <Dialog
        open={aboutMeDialog}
        onClose={() => handleAboutMeDialogClose()}
            ...
            <Grid size={{ xs: 12 }}>
              <TextField
                ...
                onChange={event =>
                  setSelectedAboutMe({
                    ...selectedAboutMe,
                    introducingLine: event.target.value,
                  })
                }
              />
            </Grid>
          ...
          >
            <Grid>
              <Button
                variant="contained"
                color="error"
                onClick={() => handleAboutMeDialogClose()}
              >
                Cancel
              </Button>
            </Grid>
