...
const RightElement: React.FC<ElementProps> = ({ id, children }) => {
  return (
    <Grid size={{ xs: 12 }} id={id} style={{ minHeight: '100dvh' }}>
      {children}
    </Grid>
  );
};
  ...
  return (
    <Grid container>
      ...
      {(isXl || isLg || isMd || isSm || isXs) && (
        <Grid
          container
          size={{ xl: 7, lg: 7, md: 7, sm: 12, xs: 12 }}
          style={{ overflowY: 'auto', height: '100dvh' }}
          className={`pl-5 ${isXs || isSm ? 'pr-5' : 'pr-10'}`}
        >
          {(isXs || isSm) && mobileElement && (
            <MobileNavElement id="section-mobile-home">
              {mobileElement}
            </MobileNavElement>
          )}
          {rightElements &&
            rightElements.map((webElement, index) => (
              <RightElement key={index} id={`section${index}`}>
                {webElement}
              </RightElement>
            ))}
        </Grid>
      ...
      <ScrollToTop
        source="app"
        scrollToIdLarge="section0"
        scrollToIdSmall="section-mobile-home"
      />
    </Grid>
  );
};
